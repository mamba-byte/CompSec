...
foreach (['pot', 'run-name'] as $required) {
    if (!isset($options[$required])) {
        fwrite(STDERR, "Usage: apply_hashcat_results.php --pot hashcat.pot --run-name exp1 [--wordlist rockyou.txt --hashes hashes.txt --duration 123.4]\n");
        exit(1);
    }
}
...
$hashFile = $options['hashes'] ?? 'hashcat/hashes.txt';
...
$updated = $db->applyPotfile($potfile, $runId, $duration);

fprintf(STDOUT, "Applied potfile %s. Updated %d rows (run id %d).\n", $potfile, $updated, $runId);
